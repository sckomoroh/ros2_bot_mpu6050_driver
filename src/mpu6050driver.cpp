    ...
    , imu_frame_{"imu_frame"}
{
    ...
    imu_topic_ = get_parameter("topic").as_string();
    imu_frame_ = get_parameter("frame").as_string();

    // Create publisher
    publisher_ = this->create_publisher<sensor_msgs::msg::Imu>(imu_topic_, 10);
...
void MPU6050Driver::declareParameters()
{
    ...
    this->declare_parameter<std::string>("topic", "imu_topic");
    this->declare_parameter<std::string>("frame", "imu_frame");
}
